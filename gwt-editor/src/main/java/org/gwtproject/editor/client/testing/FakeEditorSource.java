...
/**
 * A trivial implementation of {@link EditorSource} that creates {@link FakeLeafValueEditor}
 * instances.
 *
...
public class FakeEditorSource<T> extends EditorSource<FakeLeafValueEditor<T>> {
  /**
   * Return value for {@link #getLastKnownPosition} if the editor has been passed into {@link
   * #dispose}.
   */
  public static final int DISPOSED = -2;

  /**
   * Return value for {@link #getLastKnownPosition} if the editor was not created by this
   * FakeEditorSource.
   */
  public static final int UNKNOWN = -1;

  private final Map<FakeLeafValueEditor<T>, Integer> lastKnownPosition =
      new HashMap<FakeLeafValueEditor<T>, Integer>();
  ...
  public int getLastKnownPosition(FakeLeafValueEditor<T> editor) {
    return lastKnownPosition.containsKey(editor) ? lastKnownPosition.get(editor) : UNKNOWN;
  }

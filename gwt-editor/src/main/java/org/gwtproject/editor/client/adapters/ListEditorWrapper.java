...
class ListEditorWrapper<T, E extends Editor<? super T>> extends AbstractList<T> {
   ...
   *
   * <p>The ListEditorWrapper can be reused if and only if the backing list is the same instance as
   * the passed list.
   */
   ...
   *
   * <p>This is basically the opposite from {@link #flush()}. It's used to reuse sub-editors instead
   * of recreating a ListEditorWrapper from scratch.
   */
  void refresh() {
    int i = 0;
    for (T item : backing) {

...
/**
 * Allows invalid Editor state to be propagated through an Editor hierarchy. Instances of
 * EditorError are typically created as a side effect of calling {@link EditorDelegate#recordError}.
 *
 * @see org.gwtproject.editor.client.testing.MockEditorError
 */
public interface EditorError {
  /**
   * Returns the absolute path location of the error, relative to the object that was passed into
   * the EditorDriver.
   *
  ...
  /**
   * Returns the path of the error relative to the Editor receiving the error. If the error concerns
   * the Editor that is receiving the error, this method will return an empty string.
   *
